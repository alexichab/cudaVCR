...
// Определение глобальных указателей
atom_t* dev_atoms = nullptr;
    ...
    int max_ochered_size)
{
    ...
    __shared__ float shared_BB[dir_number * 9];
    ...
    for (int i = threadIdx.x; i < dir_number * 9; i += blockDim.x) {
        shared_BB[i] = d_BB[config_ * dir_number * 9 + i];
    }
    __syncthreads();
    ...
    float ax_, ay_, az_;
    random_displacements_gpu(&ax_, &ay_, &az_, config_, shared_transform, &local_state, T);
...
{
    if (!atoms_to_update || count <= 0 || count > max_count) {
    ...
    cudaMemcpy(host_atoms, dev_atoms, Lx * Ly * Lz * sizeof(atom_t), cudaMemcpyDeviceToHost);
